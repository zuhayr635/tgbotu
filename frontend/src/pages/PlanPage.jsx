    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-slate-400">Mevcut planınız ve kullanım istatistikleri</p>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-[#16162a] rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">Plan Tipi</div>
            <div className="text-white font-semibold">{currentPlan.name}</div>
          </div>
          <div className="bg-[#16162a] rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">Maksimum Bot</div>
            <div className="text-white font-semibold">{planInfo?.max_bots || 1}</div>
          </div>
          <div className="bg-[#16162a] rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">Maksimum Grup</div>
            <div className="text-white font-semibold">{planInfo?.max_groups || 5}</div>
          </div>
          <div className="bg-[#16162a] rounded-xl p-4">
            <div className="text-slate-400 text-sm mb-1">Token / Dönem</div>
            <div className="text-white font-semibold">{planInfo?.tokens_per_period || 100}</div>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="h-3 bg-[#16162a] rounded-full overflow-hidden">
              <div
      ...
      >
        <h2 className="text-lg font-semibold text-white mb-4">Plan Yükselt</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {Object.entries(planFeatures).map(([key, plan]) => {
            ...
            return (
              ...
              >
                <div className={`bg-gradient-to-br ${planColors[key]} p-6`}>
                  {isCurrent && (
                    <div className="absolute top-4 right-4 bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-medium text-white">
                      Mevcut
                    </div>
                  )}
                  <div className="text-white/80 text-sm font-medium mb-1">{plan.name}</div>
                  <div className="text-3xl font-bold text-white mb-1">{plan.price}</div>
                  <div className="text-white/60 text-sm mb-4">{plan.period}</div>
                  ...
                  {!isCurrent && (
                    <button className="w-full py-3 bg-white/20 hover:bg-white/30 backdrop-blur-sm rounded-xl text-white font-medium transition-colors flex items-center justify-center gap-2">
                      Yükselt
      ...
      >
        ...
        <div className="text-center py-8 text-slate-400">
          ...
          <p className="text-sm mt-1">Plan yükselttiğinizde burada görünecektir</p>
        </div>
      </motion.div>
    </div>
  )
