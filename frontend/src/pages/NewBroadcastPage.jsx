    ...
    <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-slate-400 mt-1 text-sm sm:text-base">Mesajinizi yazin, gruplarınızı secin ve gonderin</p>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        >
          {/* Message Composer */}
          <div className="bg-[#1e1e3a] rounded-xl sm:rounded-2xl border border-indigo-500/20 overflow-hidden">
            {/* Toolbar */}
            <div className="flex flex-wrap items-center gap-2 px-3 sm:px-4 py-2 sm:py-3 border-b border-indigo-500/10 bg-[#16162a]/50">
              <label className="flex items-center gap-2 px-2 sm:px-3 py-2 rounded-lg text-slate-400 hover:text-indigo-400 hover:bg-indigo-500/10 transition-colors cursor-pointer text-xs sm:text-sm">
                <ImageIcon className="w-4 h-4" />
                <span className="hidden sm:inline">{media ? media.name : t('broadcast.uploadMedia')}</span>
                <span className="sm:hidden">{media ? 'Medya' : 'Yükle'}</span>
                <input type="file" accept="image/*,video/*" className="hidden" onChange={e => setMedia(e.target.files[0])} />
              </label>
              {media && (
                <button onClick={() => setMedia(null)} className="px-2 sm:px-3 py-2 rounded-lg bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 transition-colors text-xs sm:text-sm">
                  Kaldır
                </button>
              )}
              ...
              <label className="flex items-center gap-2 text-xs sm:text-sm text-slate-400 cursor-pointer">
                ...
                <span className="hidden sm:inline">{t('broadcast.disablePreview')}</span>
                <span className="sm:hidden">Link gizle</span>
              </label>
              <select value={parseMode} onChange={e => setParseMode(e.target.value)} className="bg-[#16162a] border border-indigo-500/20 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm text-slate-200 focus:outline-none">
            ...
            <textarea
              ...
              placeholder="Mesajinizi yazin... {Merhaba|Selam} gibi spintax ve degisken kullanabilirsiniz."
              rows={8}
              className="w-full bg-[#16162a] px-3 sm:px-4 py-3 sm:py-4 text-white placeholder-slate-500 resize-none focus:outline-none text-sm"
            />

            {/* Footer */}
            <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 border-t border-indigo-500/10 bg-[#16162a]/50">
              <span className="text-xs text-slate-500 hidden sm:block">
                Spintax: {'{Merhaba|Selam|Hey}'}  •  Degiskenler: {'{grup_adi}'} {'{tarih}'} {'{saat}'}
              </span>
              <span className="text-xs sm:text-sm text-slate-500">{message.length} karakter</span>
            </div>
          </div>

          {/* Schedule */}
          <div className="bg-[#1e1e3a] rounded-xl sm:rounded-2xl border border-indigo-500/20 p-4 sm:p-6">
            <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-3 sm:mb-4 flex items-center gap-2">
            ...
            <div className="flex gap-2 sm:gap-4">
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                onClick={() => setMode('now')}
                className={`flex-1 flex items-center justify-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-4 rounded-xl border transition-all duration-300 ${
                  mode === 'now'
              ...
              >
                ...
                <span className="font-medium text-xs sm:text-base">{t('broadcast.sendNow')}</span>
                {mode === 'now' && <Check className="w-4 h-4 sm:w-5 sm:h-5 ml-1 sm:ml-2" />}
              </motion.button>
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
              ...
              >
                ...
                <span className="font-medium text-xs sm:text-base">{t('broadcast.schedule')}</span>
                {mode === 'schedule' && <Check className="w-4 h-4 sm:w-5 sm:h-5 ml-1 sm:ml-2" />}
              </motion.button>
            </div>
            ...
            <AnimatePresence>
              {mode === 'schedule' && (
                ...
                >
                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-2">{t('broadcast.scheduleDate')}</label>
                    <input
                      type="datetime-local"
                      value={runAt}
                      onChange={e => setRunAt(e.target.value)}
                      className="w-full bg-[#16162a] border border-indigo-500/20 rounded-xl px-3 sm:px-4 py-2 sm:py-3 text-white input-focus transition-all duration-300 text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-2">Tekrar</label>
                    <select
                      value={repeatType}
                  ...
                  {repeatType !== 'none' && (
                    <div>
                      <label className="block text-xs sm:text-sm font-medium text-slate-300 mb-2">Tekrar bitis tarihi (opsiyonel)</label>
                      <input
                        type="datetime-local"
                        value={repeatEndAt}
            ...
            <div className="bg-[#1e1e3a] rounded-xl sm:rounded-2xl border border-indigo-500/20 p-4 sm:p-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                ...
                {broadcasting && (
                  <button
                    ...
                    className="px-3 sm:px-4 py-2 rounded-lg bg-rose-500/20 text-rose-400 hover:bg-rose-500/30 transition-colors text-xs sm:text-sm font-medium"
                  >
                    {t('broadcast.cancel')}
                  </button>
                )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mt-3 sm:mt-4">
                {[
                  ...
                  <div key={s.label} className="bg-[#16162a]/50 rounded-xl p-2 sm:p-3 text-center">
              ...
              {progress.current_title && broadcasting && (
                <div className="mt-3 sm:mt-4 flex justify-end">
                  <button
                    ...
                    className="px-3 sm:px-4 py-2 rounded-lg bg-slate-700/50 text-slate-400 hover:bg-slate-700 transition-colors text-xs sm:text-sm"
                  >
        ...
        >
          ...
          <div className="bg-[#1e1e3a] rounded-xl sm:rounded-2xl border border-indigo-500/20 p-4 sm:p-6">
            ...
            <button
              onClick={toggleAll}
              className="w-full mb-2 sm:mb-3 px-3 sm:px-4 py-2 rounded-lg bg-indigo-500/20 text-indigo-300 hover:bg-indigo-500/30 transition-colors text-xs sm:text-sm font-medium"
            >
            ...
            {tags.length > 0 && (
              <select
                ...
                className="w-full mb-2 sm:mb-3 bg-[#16162a] border border-indigo-500/20 rounded-lg px-2 sm:px-3 py-2 text-xs sm:text-sm text-slate-200 focus:outline-none"
              >
            ...
            <div className="space-y-2 max-h-60 sm:max-h-80 overflow-y-auto pr-1 sm:pr-2">
              {filtered.length === 0
                ? <p className="text-slate-400 text-xs sm:text-sm text-center py-3 sm:py-4">{t('broadcast.noGroups')}</p>
                : filtered.map(g => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`w-full flex items-center gap-2 sm:gap-3 p-2 sm:p-3 rounded-xl border transition-all duration-300 text-left ${
                        sel
                    ...
                    >
                      <div className={`w-4 h-4 sm:w-5 sm:h-5 rounded border-2 flex items-center justify-center transition-colors ${
                        ...
                        {sel && <Check className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className={`text-xs sm:text-sm font-medium truncate ${sel ? 'text-indigo-300' : 'text-slate-300'}`}>{g.title}</p>
          ...
          {/* Summary Card */}
          <div className="bg-gradient-to-br from-indigo-600/20 to-purple-600/20 rounded-xl sm:rounded-2xl border border-indigo-500/30 p-4 sm:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-white mb-3 sm:mb-4">Ozet</h3>
            <div className="space-y-2 sm:space-y-3">
              ...
              <div className="h-px bg-indigo-500/20 my-2 sm:my-3" />
              <div className="flex justify-between text-xs sm:text-sm">
          ...
          <motion.button
            ...
            className="w-full py-3 sm:py-4 rounded-xl btn-gradient text-white font-semibold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/30 text-sm sm:text-base"
          >
            <Send className="w-4 h-4 sm:w-5 sm:h-5" />
