...
export default function DashboardPage() {
  ...
  const handleCleanup = async () => {
    if (!confirm('Tüm yüklü medya dosyaları silinecek. Emin misiniz?')) return
  ...
  return (
    <div>
      ...
      {data.storage_warning && (
        <div style={{ background: '#3d2800', border: '1px solid #f59e0b', borderRadius: 10, padding: '12px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ color: '#fbbf24', fontSize: 14 }}>
            ⚠️ {t('dashboard.storageWarning', { mb: data.storage_mb })}
          </span>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 28 }}>
        <StatCard label={t('dashboard.totalGroups')} value={data.total_groups} icon="👥" />
        <StatCard label={t('dashboard.todayBroadcasts')} value={data.today_broadcasts} icon="📢" />
        <StatCard label={t('dashboard.pendingTasks')} value={data.pending_tasks} icon="⏰" />
        <StatCard
          label={t('dashboard.botStatus')}
          value={data.bot_running ? t('dashboard.botRunning') : t('dashboard.botStopped')}
          icon={data.bot_running ? '🟢' : '🔴'}
